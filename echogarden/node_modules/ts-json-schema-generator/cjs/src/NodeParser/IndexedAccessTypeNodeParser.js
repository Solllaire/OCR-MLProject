    ...
    createType(node, context) {
        ...
        const propertyTypes = indexTypes.map((type) => {
            if (!(type instanceof LiteralType_js_1.LiteralType || type instanceof StringType_js_1.StringType || type instanceof NumberType_js_1.NumberType)) {
                throw new LogicError_js_1.LogicError(`Unexpected type "${type.getId()}" (expected "LiteralType.js" or "StringType.js" or "NumberType.js")`);
            }
            ...
            if (!propertyType) {
                ...
                else if (type instanceof LiteralType_js_1.LiteralType) {
                    if (objectType instanceof ReferenceType_js_1.ReferenceType) {
                        return objectType;
                    }
                    throw new LogicError_js_1.LogicError(`Invalid index "${type.getValue()}" in type "${objectType.getId()}"`);
                }
                else {
                    throw new LogicError_js_1.LogicError(`No additional properties in type "${objectType.getId()}"`);
                }
            }
            return propertyType;
        });
