...
class TypeofNodeParser {
    ...
    createType(node, context, reference) {
        ...
        if (!valueDec) {
            ...
            throw new LogicError_js_1.LogicError(`No value declaration found for symbol "${symbol.name}"`);
        }
            ...
            else if ((initializer = valueDec?.initializer)) {
        ...
        throw new LogicError_js_1.LogicError(`Invalid type query "${valueDec.getFullText()}" (ts.SyntaxKind = ${valueDec.kind})`);
    }
    createObjectFromEnum(node, context, reference) {
        ...
        const properties = node.members.map((member) => {
            ...
            if (member.initializer) {
                ...
                throw new LogicError_js_1.LogicError(`Enum initializer missing for "${name}"`);
            }
