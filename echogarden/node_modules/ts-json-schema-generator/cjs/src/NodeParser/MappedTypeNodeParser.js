...
class MappedTypeNodeParser {
    ...
    createType(node, context) {
        ...
        else if (keyListType instanceof EnumType_js_1.EnumType) {
            return new ObjectType_js_1.ObjectType(id, [], this.getValues(node, keyListType, context), false);
        }
        else if (keyListType instanceof NeverType_js_1.NeverType) {
            return new ObjectType_js_1.ObjectType(id, [], [], false);
        }
        else {
            throw new LogicError_js_1.LogicError(`Unexpected key type "${constraintType ? constraintType.getId() : constraintType}" for type "${node.getText()}" (expected "UnionType" or "StringType")`);
        }
    }
